rule all_popoolation2:
    ...
    input:
        unpack(all_popoolation2_input),


rule all_popoolation2_raw:
    """Generate raw popoolation2 data"""
    input:
        unpack(all_popoolation2_raw_input),
...
rule popoolation2_samtools_mpileup:
    ...
    output:
        pileup=temp(
            "{interim_pool}/raw/popoolation2/{sex}.{region}.{target}.mpileup{gz}"
        ),
    input:
        unpack(popoolation2_samtools_mpileup_input),
    resources:
        runtime=cfg.ruleconf("popoolation2_samtools_mpileup").runtime,
    params:
        options=cfg.ruleconf("popoolation2_samtools_mpileup").options,
    threads: cfg.ruleconf("popoolation2_samtools_mpileup").threads
    log:
        "logs/{interim_pool}/raw/popoolation2/{sex}.{region}.{target}.mpileup{gz}.log",
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/samtools_mpileup"
...
rule popoolation2_mpileup2sync_jar:
    ...
    input:
        mpileup=__INTERIM__
        / "{group}/raw/popoolation2/{sex}.{region}.{target}.mpileup.gz",
    resources:
    ...
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/mpileup2sync_jar"


rule popoolation2_indel_filtering_identify_indel_regions:
    """Identify indel regions in mpileup file"""
    output:
        gtf="{results_pool}/raw/popoolation2.indels/{sex}.{region}.{target}.mpileup{gz}.indels.gtf",
    input:
        mpileup=__INTERIM_POOL__
        / "raw/popoolation2/{sex}.{region}.{target}.mpileup{gz}",
    resources:
        mem_mb=cfg.ruleconf(
            "popoolation2_indel_filtering_identify_indel_regions"
        ).mem_mb,
        runtime=cfg.ruleconf(
            "popoolation2_indel_filtering_identify_indel_regions"
        ).runtime,
    params:
    ...
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/indel_filtering_identify_indel_regions"
...
rule popoolation2_indel_filtering_filter_sync_by_gtf:
    ...
    output:
        sync=temp(
            "{results}/{group}/analysis/{analysis}/f{itemnum}_{filtername}_{tool}/{sex}.{region}.{target}.sync{gz}"
        ),
    input:
        unpack(get_popoolation2_filter_input),
        #sync = "{results}/{group}/raw/popoolation2/{sex}.{region}.{target}.sync{gz}",
    ...
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/indel_filtering_filter_sync_by_gtf"
...
rule popoolation2_indel_filtering_remove_indels:
    ...
    wildcard_constraints:
        ...
        filtername="select",
    resources:
        runtime=cfg.ruleconf("popoolation2_indel_filtering_remove_indels").runtime,
    params:
        options=cfg.ruleconf("popoolation2_indel_filtering_remove_indels").options,
    threads: cfg.ruleconf("popoolation2_indel_filtering_remove_indels").threads
    log:
        "logs/{results}/{group}/analysis/{analysis}/f{itemnum}_{filtername}_{tool}/{sex}.{region}.{target}.sync{gz}.log",
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/indel_filtering_remove_indels"
...
rule popoolation2_subsample_synchronized:
    ...
    log:
        "logs/{results}/{group}/analysis/{analysis}/f{itemnum}_{filtername}_{tool}/{sex}.{region}.{target}.sync{gz}.log",
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/subsample_synchronized"
...
rule popoolation2_fst_sliding:
    ...
    output:
        fst=temp(
            "{results}/{group}/analysis/{analysis}/s{itemnum}_{statname}_{tool}/{sex}.{region}.w{window_size}.s{step_size}.{target}.fst.txt.gz"
        ),
    input:
        unpack(get_popoolation2_filter_input),
    wildcard_constraints:
    ...
    log:
        "logs/{results}/{group}/analysis/{analysis}/s{itemnum}_{statname}_{tool}/{sex}.{region}.w{window_size}.s{step_size}.{target}.fst.gz.log",
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/fst_sliding"
...
rule popoolation2_fisher_test:
    ...
    output:
        fet=temp(
            "{results}/{group}/analysis/{analysis}/s{itemnum}_{statname}_{tool}/{sex}.{region}.w{window_size}.s{step_size}.{target}.fet.txt.gz"
        ),
    input:
        unpack(get_popoolation2_filter_input),
    wildcard_constraints:
    ...
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/fisher_test"
...
rule popoolation2_snp_frequency_diff:
    ...
    output:
        rc=temp(
            "{results}/{group}/analysis/{analysis}/s{itemnum}_{statname}_{tool}/{sex}.{region}.{target}.sync_rc.txt{gz}"
        ),
        pwc=temp(
            "{results}/{group}/analysis/{analysis}/s{itemnum}_{statname}_{tool}/{sex}.{region}.{target}.sync_pwc.txt{gz}"
        ),
    input:
        unpack(get_popoolation2_filter_input),
    resources:
    ...
    wrapper:
        f"{WRAPPER_PREFIX}/bio/popoolation2/snp_frequency_diff"


rule popoolation2_gather_parallel_results:
    """Gather results from parallel analyses"""
    output:
        analysis="{results}/{group}/analysis/{analysis}/s{itemnum}_{statname}_{tool}/{sex}.{region}{tag}.{suffix}",
    input:
        unpack(popoolation2_gather_parallel_results_input),
    wildcard_constraints:
        suffix="(fet.txt.gz|fst.txt.gz|sync_rc.txt.gz|sync_pwc.txt.gz)",
        tag="(|\.w\d+\.s\d+)",
        group="pool",
    resources:
        runtime=cfg.ruleconf("popoolation2_gather_parallel_results").runtime,
    params:
        options=cfg.ruleconf("popoolation2_gather_parallel_results").options,
    log:
        "logs/{results}/{group}/analysis/{analysis}/s{itemnum}_{statname}_{tool}/{sex}.{region}{tag}.{suffix}.log",
