rule all_qc:
    input:
        __REPORTS__ / "qc/multiqc.html",
        unpack(all_multiqc),


rule qc_multiqc:
    output:
        "{reports}/qc/multiqc.html",
    input:
        unpack(all_multiqc),
    resources:
        runtime=cfg.ruleconf("qc_multiqc").runtime,
    params:
        cfg.ruleconf("qc_multiqc").options,
    log:
        "logs/{reports}/qc/multiqc.log",
    wrapper:
...
rule qc_fastqc:
    output:
        html="{results}/qc/fastqc/{prefix}{bamfastq}{gz}.html",
    ...
    log:
        "logs/{results}/qc/fastqc/{prefix}{bamfastq}{gz}.log",
    wrapper:
        f"{SMK_WRAPPER_PREFIX}/bio/fastqc"


rule qc_jellyfish_count:
    output:
        jf=temp("{results}/qc/jellyfish/{sample}{trimmed}{ext}{gz}.{kmer}mer_counts.jf"),
    input:
        unpack(jellyfish_count),
    resources:
        runtime=cfg.ruleconf("qc_jellyfish_count").runtime,
    wildcard_constraints:
        ext="(.fq|.fastq|.fa|.fasta|.txt|)",
        trimmed="(|.trimmed)",
    params:
        options=cfg.ruleconf("qc_jellyfish_count").options,
    threads: cfg.ruleconf("qc_jellyfish_count").threads
    log:
        "logs/{results}/qc/jellyfish/{sample}{trimmed}{ext}{gz}.{kmer}mer_counts.log",
    wrapper:
...
rule qc_jellyfish_histo:
    output:
        hist="{prefix}.{kmer}_jf.hist",
    input:
        counts="{prefix}.{kmer}mer_counts.jf",
    resources:
        runtime=cfg.ruleconf("qc_jellyfish_histo").runtime,
    params:
        options=cfg.ruleconf("qc_jellyfish_histo").options,
    threads: cfg.ruleconf("qc_jellyfish_histo").threads
    log:
        "logs/{prefix}.{kmer}mer_counts.jf.log",
    wrapper:
...
rule qc_picard_collect_alignment_summary_metrics:
    output:
        "{results}/qc/align/{prefix}{bam}.align_metrics.txt",
    input:
        bam="{prefix}{bam}",
        ref=cfg["db"]["ref"],
    log:
        "logs/{results}/qc/align/{prefix}{bam}.align_metrics.log",
    resources:
        runtime=cfg.ruleconf("qc_picard_collect_alignment_summary_metrics").runtime,
    params:
        cfg.ruleconf("qc_picard_collect_alignment_summary_metrics").options,
    threads: cfg.ruleconf("qc_picard_collect_alignment_summary_metrics").threads
    wrapper:
        f"{SMK_WRAPPER_PREFIX}/bio/picard/collectalignmentsummarymetrics"


rule qc_picard_collect_insert_size_metrics:
    output:
        txt="{results}/qc/align/{prefix}{bam}.insert_metrics.txt",
        pdf="{results}/qc/align/{prefix}{bam}.insert_metrics.pdf",
    input:
        "{prefix}{bam}",
    log:
        "logs/{results}/qc/align/{prefix}{bam}.insert_metrics.log",
    resources:
        runtime=cfg.ruleconf("qc_picard_collect_insert_size_metrics").runtime,
    params:
        cfg.ruleconf("qc_picard_collect_insert_size_metrics").options,
    threads: cfg.ruleconf("qc_picard_collect_insert_size_metrics").threads
    wrapper:
        f"{SMK_WRAPPER_PREFIX}/bio/picard/collectinsertsizemetrics"


rule qc_picard_mark_duplicates:
    output:
        metrics="{interim}/map/{aligner}/dedup/{sample}{bam}.dup_metrics.txt",
        bam="{interim}/map/{aligner}/dedup/{sample}{bam}",
    input:
        bam="{interim}/map/{aligner}/{sample}{bam}",
        bai="{interim}/map/{aligner}/{sample}{bam}.bai",
    log:
        "logs/{interim}/qc/align/{aligner}/{sample}{bam}.dup_metrics.log",
    resources:
        runtime=cfg.ruleconf("qc_picard_mark_duplicates").runtime,
        mem_mb=cfg.ruleconf("qc_picard_mark_duplicates").mem_mb,
    params:
        cfg.ruleconf("qc_picard_mark_duplicates").options,
    threads: cfg.ruleconf("qc_picard_mark_duplicates").threads
    wrapper:
        f"{WRAPPER_PREFIX}/bio/picard/markduplicates"
...
rule qc_qualimap_bamqc_pe:
    ...
    input:
        bam=map_sample_target,
    params:
        options=cfg.ruleconf("qc_qualimap_bamqc_pe").options,
    resources:
        runtime=cfg.ruleconf("qc_qualimap_bamqc_pe").runtime,
        mem_mb=cfg.ruleconf("qc_qualimap_bamqc_pe").mem_mb,
    threads: cfg.ruleconf("qc_qualimap_bamqc_pe").threads
    log:
        "logs/{results}/qc/qualimap/{sample}{bam}.pe.qualimap.log",
    wrapper:
        f"{WRAPPER_PREFIX}/bio/qualimap/bamqc"
...
rule qc_qualimap_bamqc_se:
    ...
    input:
        bam=map_sample_target,
    params:
        options=cfg.ruleconf("qc_qualimap_bamqc_se").options,
    resources:
        runtime=cfg.ruleconf("qc_qualimap_bamqc_se").runtime,
        mem_mb=cfg.ruleconf("qc_qualimap_bamqc_se").mem_mb,
    threads: cfg.ruleconf("qc_qualimap_bamqc_se").threads
    log:
        "logs/{results}/qc/qualimap/{sample}{bam}.se.qualimap.log",
    wrapper:
        f"{WRAPPER_PREFIX}/bio/qualimap/bamqc"
...
rule qc_sambamba_depth:
    output:
        bed="{results}/qc/sambamba/{sample}.depth.w{window_size}.bed{gz}",
    input:
        bam=map_sample_target,
        bai=map_sample_target_bai,
    resources:
        runtime=cfg.ruleconf("qc_sambamba_depth").runtime,
        mem_mb=cfg.ruleconf("qc_sambamba_depth").mem_mb,
    params:
        window_size=lambda wildcards: wildcards.window_size,
    threads: cfg.ruleconf("qc_sambamba_depth").threads
    log:
        "logs/{results}/qc/sambamba/{sample}.depth.w{window_size}.bed{gz}.log",
    wrapper:
        f"{WRAPPER_PREFIX}/bio/sambamba/depth"


rule qc_bcftools_stats:
    """Run bcftools stats to generate caller statistics on single files"""
    output:
        stats="{results}/qc/variants/{group}/{callset}/{caller}/{stage}/{region}{vartype}.vcf{gz}.stats",
    input:
        vcf="{results}/{group}/{callset}/{caller}/{stage}/{region}{vartype}.vcf{gz}",
        ref=cfg["db"]["ref"],
    wildcard_constraints:
        stage="(unfiltered|filter|select)",
        vartype="(|.indel|.snp)",
    resources:
        runtime=cfg.ruleconf("qc_bcftools_stats").runtime,
        mem_mb=cfg.ruleconf("qc_bcftools_stats").mem_mb,
    threads: 1
    log:
        "logs/{results}/qc/bcftools/{group}/{callset}/{caller}/{stage}/{region}{vartype}.vcf{gz}.stats.log",
    wrapper:
        f"{WRAPPER_PREFIX}/bio/bcftools/stats"
