...
rule popoolation_filter_pileup_by_gtf:
    ...
    output:
        pileup = temp("{results}/pool/analysis/{analysis}/{filternum}_{filtername}/{filtertype}.{sample}.{region}.{target}.pileup{gz}")
    input:
    ...
    log: "logs/{results}/pool/analysis/{analysis}/{filternum}_{filtername}/{filtertype}.{sample}.{region}.{target}.pileup{gz}.log"
...
rule popoolation_subsample_pileup:
    ...
    log: "logs/{results}/pool/analysis/{analysis}/{filternum}_{filtername}/{filtertype}.{sample}.{region}.{target}.pileup{gz}"
    wrapper: f"{WRAPPER_PREFIX}/bio/popoolation/subsample_pileup"


rule popoolation_variance_sliding:
    output: txt = temp("{results}/pool/analysis/{analysis}/{statname}/{sample}.{region}.w{window_size}.s{step_size}.{measure}.{target}.txt.gz")
    input: unpack(get_popoolation_stat_input)
    wildcard_constraints:
        measure = "(pi|theta|D)",
        filters = "([\.a-z]+|)"
    resources:
    ...
    log: "logs/{results}/pool/analysis/{analysis}/{statname}/{sample}.{region}.w{window_size}.s{step_size}.{target}.{measure}.txt.gz"
...
rule popoolation_gather_parallel_results:
    ...
    output:
        analysis = "{results}/pool/analysis/{analysis}/{statname}/{sample}.{region}.w{window_size}.s{step_size}.{measure}.txt.gz"
    ...
    log: "logs/{results}/pool/analysis/{analysis}/{statname}/{sample}{region}.w{window_size}.s{step_size}.{measure}.txt.gz.log"
