def all_multiqc(wildcards):
    val = {
        ...
        "sambamba": all_sambamba_depth(
            wildcards
        ),  # Should go to report.smk as we need to make separate plot of this
    }
    return val
...
def all_fastqc(wildcards):
    ...
    # Regular input reads
    qc = [
        f"{str(__RESULTS__)}/qc/fastqc/{x}_fastqc.zip"
        for x in reads.data["reads"].tolist()
    ]
    if cfg.workflow["trim"]:
        qc = qc + [
            f"{str(__RESULTS__)}/qc/fastqc/{x}_fastqc.zip"
            for x in all_trim(wildcards)["reads"]
        ]
    return qc
...
def jellyfish_count(wildcards):
    ...
    if wildcards.trimmed == ".trimmed":
        seq = (
            df.data["reads"]
            .str.replace(str(__RAW__), str(__INTERIM__ / "map/trim"))
            .tolist()
        )
    else:
...
def all_jellyfish(wildcards):
    ...
    for kmer in cfg.ruleconf("qc_jellyfish").extra["kmer"]:
        hist = expand(
            str(__RESULTS__ / "qc/jellyfish/{sample}{trim}.{kmer}_jf.hist"),
            sample=allsamples.samples,
            trim=trim,
            kmer=kmer,
        )
        val.extend(hist)
...
def all_picard_alignqc(wildcards):
    ...
    align_metrics = [
        "{}/qc/align/{}.align_metrics.txt".format(str(__RESULTS__), x) for x in inbam
    ]
    insert_metrics = [
        "{}/qc/align/{}.insert_metrics.txt".format(str(__RESULTS__), x) for x in inbam
    ]
...
def all_qualimap_bamqc(wildcards):
    ...
    inbam = all_map_sample_targets(wildcards)
    bamdf = {
        os.path.splitext(os.path.basename(x))[0]: os.path.basename(x) for x in inbam
    }
...
def all_bcftools_stats(wildcards):
    ...
    bn = [
        re.sub(rf"^{str(__RESULTS__)}", str(__RESULTS__ / "qc/variants"), x)
        for x in invcf
    ]
