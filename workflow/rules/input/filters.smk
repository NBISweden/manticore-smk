...
def get_filter_input(wildcards, **kwargs):
    ...
    val['vcf'] = f"{wildcards.results}/{wildcards.group}/analysis/{wildcards.analysis}/{num}_{previous_step}_{previous_filter}/{wildcards.region}.vcf.gz"
    val['tbi'] = f"{val['vcf']}.tbi"
    return val
...
def get_plot_input(wildcards):
    ...
    previous_step = list(config[analysis]["filters"].keys())[num - 1]
    previous_filter = config[analysis]["filters"][last_step]["filter"]
    num = str(count).zfill(2)
    val['vcf'] = f"{wildcards.results}/{wildcards.group}/{analysis}/{num}_{previous_step}_{previous_filter}/{wildcards.region}.vcf.gz"
    val['tbi'] = f"{val['vcf']}.tbi"
    return val
...
def get_popoolation_filter_input(wildcards, **kwargs):
    ...
    previous_step = list(config[analysis]["filters"].keys())[count - 2]
    previous_filter = config[analysis]["filters"][previous_step]["filter"]
    val['pileup'] = f"{wildcards.results}/pool/analysis/{wildcards.analysis}/{num}_{previous_step}_{previous_filter}/{wildcards.sample}.{wildcards.region}.{wildcards.target}.pileup.gz"
    return val
