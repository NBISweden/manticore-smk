...
rule all_variantfilter:
    input: all_variantfilter


rule filter_gatk_select_variants:
    """Run GATK SelectVariants to select SNPs"""
    output: vcf = "{results}/{group}/{callset}/{caller}/select/{region}.{vartype}.bcf",
            idx = "{results}/{group}/{callset}/{caller}/select/{region}.{vartype}.bcf.idx"
    input: vcf = "{results}/{group}/{callset}/{caller}/unfiltered/{region}.bcf",
           idx = "{results}/{group}/{callset}/{caller}/unfiltered/{region}.bcf.idx",
           ref = config['db']['ref']
    ...
    log: "logs/{results}/{group}/{callset}/{caller}/select/{region}.{vartype}.bcf.log"
...
rule filter_gatk_variant_JEXL_filtration:
    ...
    """
    output: vcf = "{results}/{group}/{callset}/{caller}/filter/{region}.{vartype}.jexl.bcf"
    input: vcf = "{results}/{group}/{callset}/{caller}/select/{region}.{vartype}.bcf",
           idx = "{results}/{group}/{callset}/{caller}/select/{region}.{vartype}.bcf.idx",
           ref = config['db']['ref']
    wildcard_constraints:
        vartype = "(snp|indel)"
